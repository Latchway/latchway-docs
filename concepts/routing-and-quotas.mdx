...
<Frame caption="The client names a stable feature. Latchway owns eligibility, quota preflight, route choice, fallback, and final settlement.">

  <img
    src="/assets/feature-routing-overview.svg"
    alt="A client names a stable feature; Latchway applies trust, model-aware input preflight, quotas, and retry policy before selecting a weighted or sticky route and recording each permitted upstream attempt."
  />
...
Server 1.0.4 adds local Chat Completions function tools to this profile. Tool
definitions, expanded parameter schemas, assistant calls and correlated results
contribute to the reservation. It supports text-only content, acyclic local
JSON-pointer schema references, and complete call/result exchanges. Remote or
recursive references, hosted/custom tools, media and unknown provider fields
remain rejected. The application executes tools; every generation in its loop
is a separate quota-accounted request. See [React Native framework setup](/clients/react-native/framework-integrations).
...
### Rejected requests and unknown usage

From server 1.1.2, a confirmed OpenRouter request-validation rejection before
generation releases its token and cost reservations. It still counts as a
logical request and an upstream attempt. An HTTP error alone is not sufficient
evidence: untyped errors, timeouts and ambiguous interrupted responses retain
conservative accounting.

The Console and CLI distinguish reported measurements from unknown quota
charges. The Admin API provides `usage.details` with recorded, reported and
unknown units for each token/cost metric. An unavailable measurement is `null`,
not a reported zero. A calculated zero-token settlement after rejection is not
proof that OpenRouter billed zero dollars.

Attempt diagnostics may include a sanitized provider category, structural
parameter path and generation ID. Raw provider messages and prompt bodies are
not retained. Use a confirmed rejection's `request_invalid` response to correct
the request before retrying; an operator can inspect the safe diagnostics.

### Explaining a large input reservation

`input_accounting_breakdown` exposes aggregate rewritten request bytes, framing
unit count, per-request/per-unit framing allowances and expanded schema bytes.
Tool schemas can increase both framing and schema-expansion allowances. These
are conservative bounds, not exact tokenizer measurements or prompt text.
When simulating Chat or Responses input, include `expanded_schema_bytes`; the
CLI accepts `--expanded-schema-bytes`. Historical attempts may lack this
breakdown and cannot be reconstructed from their body hash alone.
