...
| --- | --- |
| Docker Compose | Source-built candidate image, explicit migration service, and PostgreSQL |
| Google Cloud Run | Private Cloud SQL, Secret Manager, HTTPS service, and migration job |
| AWS | ECS/Fargate, private RDS, Secrets Manager, and HTTPS ALB |
| Fly.io | Fly Machines, attached PostgreSQL, and release-command migrations |
| Cloudflare | Streaming Worker shim around the same container application |
...
For a complete provider walkthrough, use [Google Cloud Run](/operations/deploy-google-cloud-run)
or [Cloudflare Containers](/operations/deploy-cloudflare-containers). Both
guides preserve GHCR as the canonical release registry. Cloud Run can import the
public GHCR image directly. Cloudflare cannot pull from GHCR, so its guide
mirrors the verified `linux/amd64` child into Cloudflare Registry without
rebuilding it.
