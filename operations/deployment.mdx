---
title: "Deployment"
description: "Plan a Latchway deployment around one verified image, PostgreSQL, and production secret custody."
icon: "server-cog"
audience: "operator"
pageType: "runbook"
serverVersion: "1.0.0"
...
Start with [Choose a deployment target](/operations/choose-deployment) to compare
availability, registry, residency, scaling, and cost boundaries. When release
coordinates exist, [verify the release image](/operations/verify-release-image)
before a cloud platform, mirror, or migration job receives it.
...
| [Google Cloud Run](/operations/deploy-google-cloud-run) | Manual image deployment, existing TLS PostgreSQL or Cloud SQL, Secret Manager, HTTPS service, and migration job; Terraform optional |
...
Each platform in the table has a source-based walkthrough. Every guide preserves
...
Docker Compose and manual Google Cloud Run are the initial v1 deployment
priorities. AWS, Fly.io, and Cloudflare remain later provider-verification
targets. An adopter does not need to run the project's release-evidence
workflows or provision a GitHub WIF identity to deploy the published image.
...
Prefer private PostgreSQL networking; an external managed endpoint requires
verified TLS and provider access controls. Restrict `/metrics` to the scraper
or private network.
...
1. Resolve the image by immutable digest and
   [verify its signature, provenance, and SPDX attestations](/operations/verify-release-image).
2. Back up PostgreSQL and prove a restore in isolation.
3. Run `migrate up`, then require `migrate status` to report `up_to_date=true`
   and `current` equal to `available`.
4. Require `/healthz` and `/readyz`; readiness includes quota-completion
   capacity and required worker-heartbeat state.
5. Keep the platform request timeout longer than the maximum supported stream.
6. Match `SIGTERM` draining to the platform deadline: normally at least the
   configured 30-second default, or the Cloud Run guide's explicit `8s` drain
   inside its ten-second shutdown window.
