...
def rendered_snippet(
    ...
    if bundle["source_tree_clean"]:
        note = (
            ...
            f"documentation source commit `{source['commit']}` for SDK `{source['release']}`. "
            "The source commit may be a documentation-only follow-up to the immutable release tag. "
...
def main() -> int:
    ...
    for bundle_index, raw_bundle in enumerate(bundles):
        ...
        if (
            ...
            or not isinstance(bundle["version"], str)
            or len(bundle["version"]) > 64
            or VERSION.fullmatch(bundle["version"]) is None
            or bundle["release"] != f"v{bundle['version']}"
            ...
            or bundle["archive"]
            != f"docs/sdk-bundles/{sdk}/docs-bundle-{bundle['version']}.tar.gz"
        ):
            fail(f"bundles[{bundle_index}] has an invalid release coordinate")
        documents = spec["documents"] | ADDITIONAL_DOCUMENTS.get((sdk, bundle["version"]), set())
        files = bundle["files"]
        ...
        for file_index, raw_file in enumerate(files):
            ...
            expected_header = (
                ...
                f"// Documented SDK release: {source['release']}\n"
                f"// Documentation source commit: {source['commit']}\n"
        ...
        if payload_paths != sorted(documents):
